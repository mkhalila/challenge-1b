...
int main() {
    ...
    
    {
        ...
        if (basicLength == 5) {
            cout << "2: Pass - a short tour that visits 5 locations, can visit them all\n";
        } else {
            cout << "2: Fail - should be able to visit all 5 of the test locations:\n";
    ...
    
    {
        ...
        if (shouldBeOne == 1) {
            cout << "3: Pass - a tour with speed 0.001 can only visit one place\n";
        } else {
            cout << "3: Fail - a tour with speed 0.001 should only be able to visit one place, but " << shouldBeOne << " were visited\n";
            ++retval;
        }
    }
...

    {
        ...
        if (tour.size() == 34) {
            cout << "4: Pass - a larger tour correctly managed to visit 34 locations\n";
        } else {
            cout << "4: Fail - a larger tour should have visited 34 locations, but actually visited " << tour.size() << std::endl;
            ++retval;
        }
    }
